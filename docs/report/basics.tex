...
Ein neuronales Netz besteht aus \emph{Neuronen}, auch Perceptrons genannt. Neuronen nehmen \emph{Eingänge} $\mathbf{x} = (x_1, \dots, x_n)$ entgegen und berechnen mit diesen einen \emph{Ausgang} $o$.  Den Eingängen sind \emph{Gewichte} $\mathbf{w} = (w_1, \dots, w_n)$ zugeordnet; ausserdem gibt es noch einen als \emph{bias} bezeichneten Parameter $b$. Dieser Bias wird manchmal als $w_0$ mit zu $\mathbf{w}$ hinzugenommen; dann wird $\mathbf{x}$ um $x_0 = 1$ erweitert. 

Die Berechnung von $o$ geschieht mit Hilfe der \emph{Aktivierungsfunktion} $\varphi$.  Aktivierungsfunktionen sind reelle Funktionen, die meist endliche Grenzwerte für $x\to\infty$ und $x\to-\infty$ besitzen; man kann also sagen, die Aktivierungsfunktion  ist dafür zuständig, den Ausgabewert eines Neurons zu normalisieren oder einzuhegen. Die Funktionen $\tanh(x) = (e^x - e^{-x})/(e^x + e^{-x})$, $\operatorname{relu}(x) = \max\{0,x\}$ oder auch die sigmoid-Funktion $S(x) = 1/(1 + e^{-x})$ sind Beispiele häufig verwendeter Aktivierungsfunktionen.

Der Ausgang $o$ eines Neurons wird für eine gegebene Aktivierungsfunktion $\varphi$, die Gewichte $\mathbf{w}$ und die Eingabewerte $\mathbf{x}$ wie folgt berechnet:

\begin{equation}
o = \varphi(\mathbf{w} \cdot \mathbf{x}) = \varphi\bigg(\sum_{i=0}^{n} w_i x_i\bigg) = \varphi\bigg(b + \sum_{i = 1}^n w_i x_i\bigg).
\end{equation}

Der Bias $b$ dient also dazu, das Argument von $\varphi$ zu verschieben (\emph{bias}, engl.\ "`Verschiebung"'). Der Ausgang heisst auch die \emph{Aktivierung} des Neurons. 
...
Um nun ein vollständiges neuronales Netzwerk zu erhalten werden Neuronen (vgl.\ Abschnitt~\ref{basic:neural_network:neuron}) in \emph{Schichten} angeordnet. Dabei gibt es drei Schichten, welches jedes neuronale Netzwerk hat: Eine \emph{Eingabeschicht}, eine oder mehrere \emph{Hiddenschichten} oder \emph{verborgene Schichten} und eine \emph{Ausgabeschicht}; vgl.\ Abbildung~\ref{fig:schichten}.
